...
for s1_id in sample_s1_ids:
    ...
    if s1_row['business_address']:
        for token in re.findall(r'\b\w{3,}\b', s1_row['business_address'].lower())[:5]:
...
def soundex(token):
    token = token.upper()
    if not token: return "0000"
    first = token[0]
    mapping = {'BFPV': '1', 'CGJKQSXZ': '2', 'DT': '3', 'L': '4', 'MN': '5', 'R': '6'}
    code = first
    prev = next((v for k, v in mapping.items() if first in k), '0')
    for c in token[1:]:
        m = next((v for k, v in mapping.items() if c in k), '0')
        if m != prev and m != '0':
            code += m
            if len(code) == 4: break
        prev = m
